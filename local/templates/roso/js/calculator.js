$(document).ready(function () {
    ...
    $('.calc_content').on('click', '#animal_click', function(){
        var cur_class = $('.buttons_wrap .button.active').attr('data-class');

        $('.buttons_wrap .button.' + cur_class).removeClass('active');
        $('.buttons_wrap .button.' + cur_class).trigger('click');

    });

    // Переключение типа животного
    $('.buttons_wrap').on('click', '.button', function(){

        if (!$(this).hasClass('active')){
            $('.buttons_wrap .button').removeClass('active');
            $(this).addClass('active');

            var filter = $('.buttons_wrap .button.active').attr('data-filter');
            ...
            setTimeout(function () {
                $.ajax({
                    ...
                    url: '/ajax/calc/get_sections_korma.php',
                    success: function (data) {
    ...
    $('.calc_content').on('click', '#catalog_sections p', function(){
        ...
        setTimeout(function () {
            $.ajax({
                ...
                url: '/ajax/calc/get_products_korma.php',
                success: function (data) {
                    $('.calc_content').html(data);
                    $('.loading').hide();
                    $('#animal_click b').remove();
